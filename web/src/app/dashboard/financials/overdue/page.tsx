        ...
        async function loadSchedules() {
            try {
                ...
                if (res.ok) {
                    ...
                    const now = new Date();
                    const overdue = data.filter((s: any) => !s.is_paid && new Date(s.due_date) < now);
                    // Sort by oldest due date to prioritize collection
                    overdue.sort((a: any, b: any) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime());
                    setSchedules(overdue);
                }
            } catch (e) {
    ...
    return (
        <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm animate-in slide-in-from-bottom-4">
            ...
            {loading ? (
                ...
                <p className="text-sm text-slate-400 italic bg-slate-50 p-6 rounded-xl border border-dashed border-slate-200 text-center">No overdue payments. All accounts clear.</p>
            ) : (
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-slate-100">
                                ...
                                <th className="pb-4 text-[10px] font-black uppercase text-slate-400 tracking-widest px-4">Amount Overdue</th>
                                <th className="pb-4 text-[10px] font-black uppercase text-slate-400 tracking-widest px-4">Missed Date</th>
                                <th className="pb-4 text-[10px] font-black uppercase text-slate-400 tracking-widest px-4">Booking / Customer</th>
                            </tr>
                        </thead>
                        <tbody>
                            {schedules.map((s, idx) => (
                                <tr key={s.id} className="border-b border-red-50 bg-red-50/20 hover:bg-red-50/50 transition-colors">
                                    <td className="py-4 px-4 font-bold text-slate-800 text-sm whitespace-nowrap">{s.milestone}</td>
                                    <td className="py-4 px-4 font-black text-red-600 text-sm whitespace-nowrap">₹{(s.amount || 0).toLocaleString()}</td>
